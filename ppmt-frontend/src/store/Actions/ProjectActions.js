import { GET_ERRORS , GET_PROJECT, GET_PROJECTID,DELETE_PROJECT} from "../Actions/ActionTypes"
...
export const deleteProject = (projectId) => {
    return (dispatch) => {
        if (window.confirm(" Do you want to delete this project?")) {
            axios.delete(`http://localhost:8080/project/${projectId}`)
            .then((res) => {
...
export const updateProject  = (project, history) => {
    ...
    return (dispatch) => {
        axios.put("http://localhost:8080/project", project)
            .then((res) => {
                console.log("res", res);
                history.push("/dashboard");
                 dispatch(setCreateProjectError({}));
